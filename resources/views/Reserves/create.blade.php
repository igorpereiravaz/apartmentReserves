...
<body style="background-color:snow;">
    <div class="container">
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
          ...
          <form role="form" method="POST" action="{{ route('reserveStore') }}">
            {{ csrf_field() }}

            <div class="form-group">
                <label for="exampleInputEmail1">Apartamento</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ $apartment->name }}" disabled>
            </div>

            <div class="form-group">
                <label for="exampleInputEmail1">Endereço</label>
                <input type="text" class="form-control" id="name" name="name" value="{{ $apartment->address_complete.', '.
                $apartment->address_city.'-'.$apartment->address_state }}" disabled>
            </div>

            <div class="form-group">
                <input type="text" class="form-control" id="apartment_id" name="apartment_id" value="{{ $apartment->id }}" hidden>
            </div>

            <div class="form-group">
              <label for="exampleInputEmail1">Descrição da Reserva</label>
              <input type="text" class="form-control" id="description" name="description" placeholder="Penthouse Cobertura">
            </div>
